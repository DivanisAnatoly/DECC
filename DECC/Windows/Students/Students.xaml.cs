...
namespace DECC.Windows.Students
{
    ...
    public partial class Students : Window
    {
        ...
        public Students()
        {
            ...
            string query = File.ReadAllText(queriesPath + "ShowStudents.txt");

            DisplayQueryData(query);

            searchColums.ItemsSource = StudentsOptions.SearchCategories;
        }
        ...
        private void Search(object sender, RoutedEventArgs e)
        {
            ...
            StudentSearchCategories category = StudentsOptions.StringToEnum(selectedItem);
            string query = File.ReadAllText(queriesPath + "ShowStudents.txt");

            switch (category)
            {
                case StudentSearchCategories.Student:
                    query += $"  AND (\"Users\".\"Surname\" = '{searchText}' OR \"Users\".\"Name\" = '{searchText}' OR \"Users\".\"Fathername\" = '{searchText}')";
                    break;
                case StudentSearchCategories.Group:
                    query += $"  AND \"Groups\".\"Name\" = '{searchText}'";
                    break;
                case StudentSearchCategories.Profile:
